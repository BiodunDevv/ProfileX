...
interface PortfolioStore {
  ...
  updatePortfolio: (id: string, data: Partial<Portfolio>) => Promise<Portfolio | null>;
  deletePortfolio: (id: string) => Promise<boolean>;
  generateCustomUrl: (portfolioId: string, customUrl: string) => Promise<{ customUrl: string; fullUrl: string } | null>;
...
const usePortfolioStore = create<PortfolioStore>((set, get) => ({
  ...
  generateCustomUrl: async (portfolioId: string, customUrl: string) => {
    try {
      ...
      set(state => ({ 
        portfolios: state.portfolios.map(p => 
          p._id === portfolioId ? { ...p, customUrl: responseData.customUrl, isPublic: true } : p
        ),
        currentPortfolio: state.currentPortfolio?._id === portfolioId 
          ? { ...state.currentPortfolio, customUrl: responseData.customUrl, isPublic: true } 
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to generate custom URL');
      return null;
    }
  },
  ...
  checkCustomUrlAvailability: async (customUrl: string) => {
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || 'Failed to check custom URL availability');
      }
